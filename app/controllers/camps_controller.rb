class CampsController < ApplicationController
  skip_before_action :verify_authenticity_token,:only => [:create,:all,:answer, :weall]
  ...
  def create
    ...
    if name && password && name.length > 0 && password.length >0 && SuperUser.where(:name=> name).length > 0
      camp = Camp.create(:order => order,:content => content,:description => description,:startTime => startTime,:endTime => endTime,:answers => [])
      data = {:camp => camp}
